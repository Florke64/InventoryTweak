...
public class ArrangeInventory {
    ...
    private static int layoutRotation;
    ...
    public void apply() {
        ...
        ModConfig.ArrangeLayoutType layoutType = ModConfig.ArrangeLayoutType.values()[layoutRotation];
        ArrangeLayout layout = layoutType.createArrangeLayout(screenHandler);

        if (layoutType == ModConfig.ArrangeLayoutType.DISABLED || layout == null) {
            layoutRotation++;
            return;
        }
        ...
        layoutRotation++;
        if (layoutRotation >= ModConfig.ArrangeLayoutType.values().length) layoutRotation = 0;
    }
